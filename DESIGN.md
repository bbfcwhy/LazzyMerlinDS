...
### 7.2 iOS（SwiftUI）
- 遵循 Apple HIG，不強套 web 的 layout
- **`accentColor`** 設為 Petrol Ink `#416880`（Light）/ Sky Petrol `#699FC5`（Dark）
- **Display 字型**：LXGW WenKai TC（Bundle 進 app 或從 Google Fonts 下載）
- **Body 字型**：SF Pro（系統預設）+ PingFang TC（不使用 Geist，換取系統整合感）
- **Section label**：用 SF Mono（取代 Geist Mono）配合 accentColor
- **Tab bar / nav**：遵循 iOS 原生樣式，只染 accentColor
- **Unicode glyphs**：SF Symbols 有 `star.fill`（相當於 ✦）可取代使用
