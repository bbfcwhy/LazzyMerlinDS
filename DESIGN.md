...
### 7.0 套用前先選 tier（Full Brand vs Palette-Only）

**任何專案落地前，先在兩個 tier 二選一。這是定案規則，不是建議。** §7.1～§7.7 各平台指南都在「選定 tier」之後才適用。

| | **Tier 1 · Full Brand** | **Tier 2 · Palette-Only** |
|---|---|---|
| 適用 | 自己的 idea / 給自己用的 App（小工具、個人專案、portfolio、玩具） | 別人的委託 / 給別人用的 App（保險業、客戶交付、B2B、給長輩用） |
| 套用範圍 | palette + typography + spacing + radius + button style + 材質（§2～§6 全部） | **只 palette**（§2 色彩系統）：wood 8 色 + earth status 3 色 + light/dark role token |
| 其餘 | 全走 LazzyMerlin | typography / spacing / radius / button / 材質全走**平台原生**（iOS SF + tab bar + NavigationStack + HIG dimensions；web 走框架預設） |
| Token import | 整個 `tokens/` 目錄 | **只 `tokens/color.json`**（檔內已標 `$extensions.com.lazzymerlin.tier = "palette"`） |
| 識別來源 | 整個 brand 氣質 | 純靠配色保留「慵懶梅林實驗室出品」的錨 |

**判準一句話**：這個 App 是「我想要它長這樣」還是「使用者需要它好用」？前者 Tier 1，後者 Tier 2。

**為什麼第 2 類只套配色**（YourlyOmamori case study，2026-06）：

1. **避免強推 brand**。把整套個性（serif、暖色紙質、特殊 button shape）套到別人的 App 上，等於借那個 App 推自己的 brand —— 使用者沒要求那種視覺，那是作者想要的、不是使用者想要的。
2. **維護成本不對等**。serif title 在 iOS NavigationStack 強制套不上、monospaced 字級跟系統 tab bar 撞、自有 button shape 跟 iOS 17+ chrome dimensions 對不齊；為了「套整套」要寫一堆 workaround，不划算。

所以 Tier 2 = **保留識別（配色），但不強迫對方接受整個 brand**。配色裡務必連 earth status 三色（Terracotta / Sage / Ochre）一起帶 —— 大地色 status 而非美式紅綠燈，正是 palette 最有辨識度的部分。

> Tier 2 的 token 怎麼只取 palette、機器化篩選範例見 [`tokens/README.md`](tokens/README.md)。
> 實作參照：YourlyOmamori 已選 Tier 2，`Theme.swift > LazzyMerlinPaletteOnlyTheme` 是中間版的 SwiftUI 實作範本。
...
| 檔名 | 用途 | 對應規範 |
|---|---|---|
| [`LMTextField.swift`](preview-ios/LazzyMerlinDSPreview/LazzyMerlinDSPreview/Tokens/LMTextField.swift) | Text input · `LMTextField`（單行）/ `LMTextEditor`（多行）· inset 材質 + `isInvalid` / `isDisabled`（ChronoPath landing 回饋補 iOS 實作） | §15.3.2 Input / Textarea |
| [`LMFormChrome.swift`](preview-ios/LazzyMerlinDSPreview/LazzyMerlinDSPreview/Tokens/LMFormChrome.swift) | 原生 `Form` / `List` 壓平成 DS 米色 · `.lmFormChrome()` / `.lmListChrome()`（容器層）+ `.lmListRow()`（逐 Section/row 清白卡，容器層蓋不到）| §15.3 Form |
...
**iOS 實作**：[`LMTextField`（單行）/ `LMTextEditor`（多行）](preview-ios/LazzyMerlinDSPreview/LazzyMerlinDSPreview/Tokens/LMTextField.swift)。包原生 `TextField` / `TextEditor` 成 DS inset 輸入框（`.scrollContentBackground(.hidden)` 去系統白底 + tokenized padding + `.tactileInset(radius:)`），文字色 `Color.ink`、placeholder 自畫 `Color.inkMuted`（不靠系統灰）。`radius` 預設 `LMRadius.xl`（16，對齊本節 md）、`LMTextEditor.minHeight` 預設 96。state：`isInvalid`（earthRed 1.5pt 外框，§15.3.8）/ `isDisabled`（變灰鎖定，§15.3.9）。收自 ChronoPath landing 回饋（原各專案重複手刻 `TextEditor + tactileInset`）。

> **原生 `Form` / `List` 壓平成 DS 米色**：iOS 想沿用系統 `Form`/`List` 時，用 [`.lmFormChrome()` / `.lmListChrome()`](preview-ios/LazzyMerlinDSPreview/LazzyMerlinDSPreview/Tokens/LMFormChrome.swift)（`.scrollContentBackground(.hidden)` + `.background(Color.bg)`）壓平**容器層**灰底。⚠️ cell 白卡是 row-level 背景，**`.listRowBackground(.clear)` 無法從容器層 propagate**（SwiftUI 限制、已實測），必須逐 `Section`/row 加配套的 `.lmListRow()`（`.listRowBackground(.clear)` + `.listRowSeparator(.hidden)`）。收自 ChronoPath landing 回饋。
